#In this updated version of the code, the for loop now creates a time array (time_array) that corresponds to the desired time period. 
# The np.arange() function is used to create this time array, with the start time given by i/samplerate, 
# the end time given by (i+time_period*samplerate)/samplerate, and the time step given by 1/samplerate. 
# The ax.plot() method is then called with freq as the x array and np.abs(freq_domain[i:i+int(time_period*samplerate)]) as the y array. 
# The label for each plot is set to the start time of the time array, which corresponds to the start time of the plot. 
# Finally, a legend is added to the plot to show the start time of each plot.
